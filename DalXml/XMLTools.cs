...
static class XMLTools
{
    ...
    public static DateTime? LoadProjectDate(string time)
    {
        string element = $"Project{time}Date";
        string filePath = $"{s_xml_dir}data-config.xml";
        var doc = XDocument.Load(filePath);
        DateTime.TryParse((doc.Descendants(element).First()).ToString(), out DateTime ProjectDate);
        doc.Save(filePath);
        return (DateTime?)ProjectDate;
    }
    #endregion
